...
def _print_case(case: ComparisonCase) -> None:
    ...
    for eps_text in EPSILONS:
        ...
        print(
            f"  {eps_text:<7}"
            f"{mp.nstr(_berger_right_form_defect(q), 10):>16}"
            f"{mp.nstr(_max_abs(q - offset), 10):>30}"
            f"{mp.nstr(_max_abs(q - _berger_one_jet_model(eps)), 10):>24}"
...
def _comparison_cases() -> tuple[ComparisonCase, ComparisonCase]:
    ...
    return (
        ...
        ComparisonCase(
            ...
            expected_ratio=("1", "-2", "-1", "2", "-2", "19", "2", "-19"),
