"""Long S7 scout using left data plus numerical right first-jet germs."""
...
from .right_germ import S7RightGermPoint, params_with_right_firstjet_germ, right_germ_spec


OUTPUT_DIR = Path("output/s7_full_moduli_firstjet_scouts")
SCOUT_VERSION = "s7-full-moduli-firstjet-scout-v1"
...
class FullModuliPoint:
    """Seven scaled coordinates for the S7 first-jet full-moduli scout."""
...
@dataclass(frozen=True)
class FullModuliSeed:
    """One deterministic scout seed."""

    index: int
    target: str
    point: FullModuliPoint


@dataclass(frozen=True)
class FullModuliResult:
    """One scout residual result."""
...
def _point_payload(point: FullModuliPoint) -> dict[str, str]:
...
def _iter_seeds(targets: tuple[str, ...], axis_count: int) -> list[FullModuliSeed]:
    """Build deterministic scout seeds."""
    axes = _grid_axes(axis_count)
    seeds: list[FullModuliSeed] = []
    index = 0
    for target in targets:
        for values in product(*axes):
            seeds.append(FullModuliSeed(index, target, FullModuliPoint(*values)))
            index += 1
    return seeds


def _left_from_point(base: ProblemParameters, point: FullModuliPoint) -> LeftEndpointParameters:
...
def _local_config(point: FullModuliPoint) -> SolverConfig:
...
def _reference_residual(target: str) -> tuple[mp.mpf, ...]:
    ...
    zero = FullModuliPoint(*(mp.zero for _ in COORDINATE_NAMES))
    seed = FullModuliSeed(-1, target, zero)
...
def evaluate_seed(seed: FullModuliSeed, calibrate: bool = True) -> FullModuliResult:
    """Evaluate one full-moduli first-jet scout seed."""
    ...
    try:
        ...
        return FullModuliResult(seed, (), mp.inf, mp.inf, mp.inf, False, 0, None, None, (0, 0), str(exc))
...
def _result_payload(result: FullModuliResult) -> dict:
...
def _parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Long S7 7D first-jet full-moduli scout.")
    ...
    parser.add_argument("--max-points", type=int, default=None, help="debug cap for smoke tests")
    parser.add_argument("--dry-run", action="store_true")
    return parser.parse_args()


def main() -> None:
    """Run the S7 first-jet full-moduli scout."""
    args = _parse_args()
    ...
    print("S7 full-moduli first-jet scout", flush=True)
    ...
    best: FullModuliResult | None = None
    executor = None
    if args.workers == 1:
        ...
        try:
            ...
            print("process executor unavailable; falling back to thread executor", flush=True)
