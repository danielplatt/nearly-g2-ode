"""Tests for higher-order diagnostics on S7 full-moduli scout minima."""
...
from experiments.s7 import full_moduli_firstjet_refine as refine
from experiments.s7 import full_moduli_firstjet_scout as scout
from experiments.shared.non_mirrored_common import _write_jsonl_event


def _point(value: str | float | int = 0) -> scout.FullModuliPoint:
    """Return a synthetic 7D full-moduli point."""
    x = mp.mpf(value)
    return scout.FullModuliPoint(x, mp.zero, mp.zero, mp.zero, mp.zero, mp.zero, mp.zero)


def _result(index: int, target: str, point: scout.FullModuliPoint, norm: str, failure: str | None = None) -> scout.FullModuliResult:
    """Return a synthetic full-moduli scout result."""
    seed = scout.FullModuliSeed(index, target, point)
...
def _candidate(index: int, target: str, point: scout.FullModuliPoint, norm: str, failure: str | None = None) -> refine.FullModuliScoutCandidate:
    """Return one synthetic full-moduli candidate."""
    result = _result(index, target, point, norm, failure)
    return refine.FullModuliScoutCandidate(result.seed, result)


def _write_scout_jsonl(path: Path, candidates: list[refine.FullModuliScoutCandidate], axis_count: int) -> None:
    """Write a tiny completed full-moduli scout checkpoint."""
...
def test_reconstruct_full_moduli_scout_candidates_from_payloads(tmp_path: Path) -> None:
    """Persisted full-moduli scout payloads should rebuild 7D candidates."""
...
def test_dry_run_reports_selected_full_moduli_minima_without_output(tmp_path: Path, capsys, monkeypatch) -> None:
    """Dry-run should select minima and avoid writing diagnostics."""
    path = tmp_path / "scouts.jsonl"
    candidates = [
        _candidate(0, "round", _point(0), "0.2"),
        _candidate(1, "round", _point(1), "0.05"),
